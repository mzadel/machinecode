...
labeltostring (FieldVariable (Dcpu.RegADataWord,label) bs) = "Register A data: " ++ bitstostring bs ++ " (" ++ (printf "0x%04x" value) ++ " == " ++ (show value) ++ ")"
    where value = bitsToWord16 bs
labeltostring (FieldVariable (Dcpu.RegBDataWord,label) bs) = "Register B data: " ++ bitstostring bs ++ " (" ++ (printf "0x%04x" value) ++ " == " ++ (show value) ++ ")"
    where value = bitsToWord16 bs
